...
use GraphQL\Utils\Utils as UtilsUtils;
   ...
   public function map()
   {
            ...
            'Query' => [
                ...
                'findHumansByDateOfBirth' => function ($value, Argument $args) {
                    ...
                    return array_filter(Characters::getHumans(), function ($human) use ($years) {
                        dump(in_array($human['dateOfBirth'], $years));
                        return in_array($human['dateOfBirth'], $years);
                    });
                },
                'findHumanByName' => function ($value, Argument $args) {
                  $name = $args['name'];

                     return array_filter(Characters::getHumans(), function ($human) use ($name) {

                        //dump(in_array($human['name'], $name));
                        return in_array($human['name'], $name);
            ...
            'Year' => [
                ...
                self::PARSE_VALUE => function ($value) {
                    if (!is_string($value)) {
                        throw new Error(sprintf('Cannot represent following value as a valid year: %s.', UtilsUtils::printSafeJson($value)));
                    }
